    ...
    ({ processName }) =>
      userMessage(
        [
          ...
          `6. **Server log** (v11 only): \`tm1_get_message_log(filter="${processName}", top=50)\` for server-side correlation. Skip on v12 — the message log endpoint is deprecated there and the tool is not registered.`,
          "",
    ...
    ({ cubeName }) =>
      userMessage(
        [
          ...
          `5. **Recent writes** (v11 only): \`tm1_get_transaction_log(cubeName="${cubeName}", top=30)\` — what changed lately. Skip on v12 — the transaction log endpoint is deprecated there and the tool is not registered.`,
          "",
    ...
    () =>
      userMessage(
        [
          ...
          "5. **Server log** (v11 only): `tm1_get_message_log(top=50)` — recent server-level events. Skip on v12, where the tool is not registered.",
          "",
