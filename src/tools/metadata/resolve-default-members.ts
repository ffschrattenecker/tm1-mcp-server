...
import { DefaultMembersBulkResultSchema } from "../schemas/items.js";
import { READ_ONLY } from "../annotations.js";
import { defineTool } from "../define-tool.js";

export const registerResolveDefaultMembers = defineTool({
  name: "tm1_resolve_default_members",
  description: [
  ...
  output: DefaultMembersBulkResultSchema,
  input: {
    items: z
      .array(
        z.object({
      ...
      .max(64)
      .describe(
        "List of {dimensionName, hierarchyName?} pairs. Capped at 64 per call to bound parallelism.",
      ),
  },
  handler: async ({ items }, tm1Client) => {
    const settled = await Promise.allSettled(
      items.map((it) =>
        tm1Client.dimensions.resolveDefaultMember(
          it.dimensionName,
          it.hierarchyName,
        ),
      ),
