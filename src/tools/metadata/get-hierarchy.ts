...
export const registerGetHierarchy = defineTool({
  ...
  input: {
    ...
    level: z
      ...
      .optional()
      .describe(
        "Exact level filter (0 = leaves, 1+ = consolidations). Combinable with levelMax.",
      ),
    levelMax: z
      .number()
      .int()
    ...
    elementType: z
      .enum(["Numeric", "String", "Consolidated", "All"])
      .optional()
    ...
    nameContains: z
      ...
      .optional()
      .describe(
        "Server-side OData substring filter (contains). Case-sensitive. Combine with other filters via AND.",
      ),
    nameStartsWith: z
      .string()
      .optional()
      .describe(
        "Server-side OData prefix filter (startswith). Case-sensitive.",
      ),
    nameRegex: z
      .string()
      .optional()
      .describe(
        "Client-side regex filter on element name (JS RegExp). Use for patterns OData cannot express. Invalid regex throws VALIDATION_ERROR.",
      ),
    topN: z
      .number()
      .int()
    ...
    compact: z
      ...
      .default(false)
      .describe(
        "Drop parents[] and children[] arrays from each element. Use for hierarchy overviews.",
      ),
  },
  handler: async (
    {
