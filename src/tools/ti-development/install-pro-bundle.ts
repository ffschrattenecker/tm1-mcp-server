...
export const registerInstallProBundle = defineTool({
  ...
  input: {
    ...
    pattern: z
      ...
      .optional()
      .describe(
        "Regex (JS) on filename. Default: matches *.pro (case-insensitive).",
      ),
    mode: z
    ...
    dryRun: z
      ...
      .default(false)
      .describe(
        "Parse + preflight only — no create/update calls. Default false.",
      ),
  },
  handler: async (
    ...
    const filenameRe = pattern
      ? compileUserRegex(pattern, undefined, "pattern")
      : /\.pro$/i;
    ...
    if (files.length === 0) {
      return {
        content: [
          {
            type: "text" as const,
            text: JSON.stringify({ directory, filesFound: 0, results: [] }),
          },
        ],
      };
    }
      ...
      try {
        ...
        if (!processName) {
          results.push({
            ...
            processName: null,
            status: "error",
            error: "Process name not found in .pro",
          });
        ...
        if (mode === "create" && exists) {
          results.push({
            ...
            processName,
            status: "error",
            error: `Already exists; mode=create`,
          });
          if (!continueOnError) stopped = true;
          continue;
        }
        if (mode === "update" && !exists) {
          results.push({
            ...
            processName,
            status: "error",
            error: `Does not exist; mode=update`,
          });
          if (!continueOnError) stopped = true;
          continue;
        }
    ...
    const summary = {
      ...
      counts: {
        ...
        preflight_failed: results.filter((r) => r.status === "preflight_failed")
          .length,
