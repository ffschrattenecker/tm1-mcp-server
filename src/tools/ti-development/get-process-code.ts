...
import { ProcessCodeSchema } from "../schemas/items.js";
...
export const registerGetProcessCode = defineTool({
  ...
  handler: async (
    { processName, stripComments, maskSecrets, includeSecurityAccess },
    tm1Client,
  ) => {
    ...
    if (includeSecurityAccess) {
      hasSecurityAccess = (await tm1Client.processes.getDeployMeta(processName))
        .hasSecurityAccess;
    }
