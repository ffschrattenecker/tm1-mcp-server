...
import { AuditComplexityResultSchema } from "../schemas/items.js";
...
export const registerAuditComplexity = defineTool({
  name: "tm1_audit_complexity",
  description:
  ...
  output: AuditComplexityResultSchema,
  input: {
    scope: z
  ...
  handler: async (
    ...
    if (want("processes") || want("consistency") || want("antipatterns")) {
      ...
      for (const p of all) {
        ...
        processMetrics.push(computeProcessMetrics(p.name, code, scoreWeights));
        if (want("antipatterns")) {
          findings.push(...lintProcess(p.name, code, { cellGetDimThreshold }));
        }
      }
      if (want("consistency")) {
      ...
      content: [
        {
          ...
          text: JSON.stringify({
            ...
            summary: {
              ...
              rules: {
                totalRulesLoc: rulesMetrics.reduce((a, m) => a + m.rulesLoc, 0),
