...
describe.skipIf(!LIVE_ENABLED)(
  "live: ti-extra (.pro roundtrip + ops gap)",
  () => {
    let h: LiveHarness;
    let proContent = "";
    let bundleDir = "";
    ...
    it("export_process_to_pro returns inline .pro content", async () => {
      const r = await h.ok("tm1_export_process_to_pro", {
        processName: PROC_SRC,
      });
    ...
    it("install_pro_bundle dry-run parses a directory without writing", async () => {
      ...
      await fs.writeFile(
        path.join(bundleDir, `${PROC_SRC}.pro`),
        proContent,
        "utf8",
      );
    ...
    it("get_error_log_content fetches a real log or errors cleanly on a bogus name", async () => {
      ...
      if (filename) {
        const r = await h.ok("tm1_get_error_log_content", {
          filename,
          tail: 20,
        });
        expect(r.isError).toBe(false);
