  ...
  () => {
    ...
    afterAll(async () => {
      ...
      if (createdClient) {
        ...
        try {
          await h.call("tm1_delete_client", {
            clientName: CLIENT_NAME,
            confirm: CLIENT_NAME,
          });
        } catch {
    ...
    it("tm1_list_sessions returns pagination envelope", async () => {
      const r = await h.ok("tm1_list_sessions", {
        limit: 10,
        withThreads: false,
      });
      expect(r.json).toMatchObject({
    ...
    it("FILE lifecycle: upload -> read back -> delete", async () => {
      ...
      const del = await h.ok("tm1_delete_file", {
        fileName: FILE_NAME,
        confirm: FILE_NAME,
      });
      ...
      const after = await h.call("tm1_get_file_content", {
        fileName: FILE_NAME,
      });
    ...
    it("CLIENT lifecycle: create -> update -> get -> assign/remove group -> delete", async () => {
      ...
      const del = await h.ok("tm1_delete_client", {
        clientName: CLIENT_NAME,
        confirm: CLIENT_NAME,
      });
